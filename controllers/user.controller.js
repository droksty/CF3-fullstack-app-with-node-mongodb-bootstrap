...
exports.findAll = function(req, res) {
    ...
    User.find({}, (err, results) => {
        if (err) {
            ...
            logger.log("Error in reading all users", err);
...
exports.findOne = function(req, res) {
    ...
    User.findOne({ username: username }, (err, results) => {
        if (err) {
            res.status(400).json({status: false, data: err});
            logger.log(`Error in finding user with username ${username}`, err);
...
exports.create = function(req, res) {
    ...
    newUser.save((err, result) => {
        if (err) {
            res.status(400).json({status: false, data: err})
            logger.log('Error in creating user', err);
...
exports.update = function(req, res) {
    ...
    User.findOneAndUpdate({username: username}, updateUser, {new: true}, (err, result) => {
        if (err) {
            res.status(400).json({status: false, data: err});
            logger.log('Error in updating user', err);
